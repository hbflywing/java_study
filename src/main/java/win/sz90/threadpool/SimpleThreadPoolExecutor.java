...
public class SimpleThreadPoolExecutor extends Thread {
    ...
    private SimpleThreadPoolExecutor(int minSize, int activeSize, int maxSize, int queueSize, DiscardPolicy discardPolicy) {
    ...
    void submit(Runnable runnable) {
        ...
        synchronized (TASK_QUEUE) {
            ...
            TASK_QUEUE.addLast(runnable); // 1.将任务添加到队列
            TASK_QUEUE.notifyAll(); // 2.唤醒等待的线程去执行任务
        }
    }
